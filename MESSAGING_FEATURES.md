...
**Lấy Thread ID (ID cuộc hội thoại):**
```bash
...
# Output sẽ có: threadId, userId, hoặc conversation ID
```
...
```bash
npm run msg <threadId> "Nội dung tin nhắn"
```

**Ví dụ:**
```bash
npm run msg 123456789 "Xin chào, bạn khỏe không?"
npm run msg 987654321 "Hôm nay thời tiết đẹp lắm"
```

**Hoặc dùng node trực tiếp:**
```bash
node message-sender.js 123456789 "Hello World!"
```
...
```bash
npm run msg <threadId> "Mô tả ảnh" --image /đường/dẫn/ảnh.jpg
```

**Ví dụ:**
```bash
npm run msg 123456789 "Xem ảnh này" --image ~/Documents/photo.jpg
npm run msg 987654321 "Báo cáo" --image ~/reports/report.png
```
...
```bash
npm run msg <threadId> "Mô tả file" --file /đường/dẫn/file.pdf
```

**Ví dụ:**
```bash
npm run msg 123456789 "Báo cáo đầu tháng" --file ~/reports.docx
npm run msg 987654321 "Bảng tính" --file ~/data.xlsx
```
...
```bash
npm run msg <threadId> "Mô tả link" --link https://example.com
```
